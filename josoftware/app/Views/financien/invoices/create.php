...
<div class="flex justify-between items-center mb-2">
    <h2 style="font-size:1.1rem;font-weight:700;">Nieuwe factuur</h2>
    <a href="<?= APP_URL ?>/financien/facturen" class="btn btn-secondary btn-sm">Terug naar facturen</a>
</div>

<form method="POST" action="<?= APP_URL ?>/financien/facturen/nieuw" id="invoiceForm">
    <?= CSRF::field() ?>

    <!-- ── Factuurdetails ────────────────────────────────────────────────── -->
    <div class="card" style="margin-bottom:1rem;">
        <div class="card-header"><span class="card-title">Factuurgegevens</span></div>
        <div class="card-body">
            <div class="form-row">
                ...
                <div class="form-group">
                    <label class="form-label" for="issue_date">Factuurdatum <span style="color:#ef4444;">*</span></label>
                    <input type="date" name="issue_date" id="issue_date" class="form-input"
                           value="<?= date('Y-m-d') ?>" required>
                </div>
                <div class="form-group">
                    <label class="form-label" for="due_date">Vervaldatum <span style="color:#ef4444;">*</span></label>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label" for="quote_id">Gekoppelde offerte (optioneel)</label>
                    <select name="quote_id" id="quote_id" class="form-select w-full">
                        <option value="">— Geen offerte gekoppeld —</option>
                        <?php foreach ($acceptedQuotes as $q): ?>
                            <option value="<?= (int) $q['id'] ?>">
                                <?= htmlspecialchars($q['quote_number'] . ' — ' . $q['company_name'], ENT_QUOTES, 'UTF-8') ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
    ...
    <div class="flex gap-1" style="justify-content:flex-end;">
        <a href="<?= APP_URL ?>/financien/facturen" class="btn btn-secondary">Annuleren</a>
